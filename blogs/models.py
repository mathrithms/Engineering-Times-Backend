...
class Author(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        self.full_clean()
        super(Author, self).save(force_insert,
...
class Blog(models.Model):
    ...
    author = models.ForeignKey(
        Author, on_delete=models.PROTECT, related_name='blogs')
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        self.full_clean()
        super(Blog, self).save(force_insert,
                               force_update, using, update_fields)
        file_name = self.thumbnail.name.split('/')[-1]
        cdn_thumbnail = resize_upload(file_name, 'BlogThumbnail', str(
            self.id) + '_' + str(int(time.time())), (80, 45))
        if cdn_thumbnail is not None:
            self.thumbnail = cdn_thumbnail

        file_name = self.main_image.name.split('/')[-1]
        cdn_main_image = resize_upload(file_name, 'BlogMain', str(
            self.id) + '_' + str(int(time.time())))
...
class ContentBlock(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        cdn_image = resize_upload(file_name, 'ContentBlock', str(
            self.id) + '_' + str(int(time.time())))
...
class Reference(models.Model):
    ...
    blog = models.ForeignKey(
        Blog, on_delete=models.PROTECT, related_name='references')
    ...
    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        self.full_clean()
        super(Reference, self).save(force_insert,
                                    force_update, using, update_fields)
...
class Recommended(models.Model):
    tier1 = models.ForeignKey(
        Blog, on_delete=models.PROTECT, related_name='tier1')
    tier1_image = models.ImageField()
    tier2 = models.ForeignKey(
        Blog, on_delete=models.PROTECT, related_name='tier2')
    tier2_image = models.ImageField()
    tier3 = models.ForeignKey(
        Blog, on_delete=models.PROTECT, related_name='tier3')
    tier3_image = models.ImageField()
    tier4 = models.ForeignKey(
        Blog, on_delete=models.PROTECT, related_name='tier4')
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        if not Recommended.objects.exists():
            self.full_clean()
            super(Recommended, self).save(force_insert,
                                          force_update, using, update_fields)
